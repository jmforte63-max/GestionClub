...
export const crearConceptoIngreso = async (req, res) => {
  try {
    ...
    const [existing] = await pool.query(
      'SELECT id FROM conceptos_ingresos WHERE club_id = ? AND nombre = ? LIMIT 1',
      [clubId, nombreTrim]
    );
    ...
    const [result] = await pool.query(
      'INSERT INTO conceptos_ingresos (club_id, nombre, descripcion, iva, activo) VALUES (?, ?, ?, ?, ?)',
    ...
    res.status(201).json({
      ...
      mensaje: 'Concepto de ingreso creado correctamente'
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const obtenerConceptoIngreso = async (req, res) => {
  try {
    const { id } = req.params;
    const selectedClubId = canSeeAllClubs(req) ? getSelectedClubId(req) : null;
    const clubId = selectedClubId ?? getClubId(req);
    ...
    const [rows] = await pool.query(
      selectedClubId !== null || !canSeeAllClubs(req)
        ? 'SELECT * FROM conceptos_ingresos WHERE id = ? AND club_id = ?'
        : 'SELECT * FROM conceptos_ingresos WHERE id = ?',
...
export const actualizarConceptoIngreso = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const [rows] = await pool.query(
      selectedClubId !== null || !canSeeAllClubs(req)
        ? 'SELECT * FROM conceptos_ingresos WHERE id = ? AND club_id = ? LIMIT 1'
        : 'SELECT * FROM conceptos_ingresos WHERE id = ? LIMIT 1',
      selectedClubId !== null || !canSeeAllClubs(req) ? [id, clubId] : [id]
    );
    ...
    await pool.query(
      selectedClubId !== null || !canSeeAllClubs(req)
        ? 'UPDATE conceptos_ingresos SET nombre = ?, descripcion = ?, iva = ?, activo = ? WHERE id = ? AND club_id = ?'
        : 'UPDATE conceptos_ingresos SET nombre = ?, descripcion = ?, iva = ?, activo = ? WHERE id = ?',
      selectedClubId !== null || !canSeeAllClubs(req)
    ...
    res.json({ mensaje: 'Concepto de ingreso actualizado correctamente' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

export const eliminarConceptoIngreso = async (req, res) => {
  try {
    const { id } = req.params;
    const selectedClubId = canSeeAllClubs(req) ? getSelectedClubId(req) : null;
    const clubId = selectedClubId ?? getClubId(req);
    ...
    await pool.query(
      selectedClubId !== null || !canSeeAllClubs(req)
        ? 'DELETE FROM conceptos_ingresos WHERE id = ? AND club_id = ?'
        : 'DELETE FROM conceptos_ingresos WHERE id = ?',
      selectedClubId !== null || !canSeeAllClubs(req) ? [id, clubId] : [id]
    );

    res.json({ mensaje: 'Concepto de ingreso eliminado correctamente' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
